...
func ConnectServerByProxyWithTLS(proxyURL string, Websocket_domain string, protocol string, addr string, tlsConfig *tls.Config, disableCustomTLSHeadByte bool) (c net.Conn, err error) {
	c, err = ConnectServerByProxy(proxyURL, Websocket_domain, protocol, addr)
	if err != nil {
		return
	}
